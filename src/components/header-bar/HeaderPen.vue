...
export default {
  ...
  methods: {
    ...
    sliderChange: function(value) {
      this.opacity = value
      this.PEN_SET_OPACITY(value / 100)
    },
    widthChange: function(value) {
      this.width = value
      this.PEN_SET_WIDTH(value)
    },
    colorChange: function(value) {
      if(this.colorType == 'STROKE') {
        this.strokeColor = value
        this.PEN_SET_STROKECOLOR(value)
      }
      else {
        this.fillColor = value
        this.PEN_SET_FILLCOLOR(value)
      }
    }
  },
  watch: {
    colorType: function(_new, _old) {
      if(_new == 'STROKE') {
        this.color = this.PEN_STROKECOLOR
      }
      else {
        this.color = this.PEN_FILLCOLOR
      }
    }
  },
  mounted: function() {
    ...
    this.width = this.PEN_WIDTH
    this.opacity = this.PEN_OPACITY * 100
  }
}
</script>
